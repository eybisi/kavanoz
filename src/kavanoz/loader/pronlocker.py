        ...
        for d in self.dvms:
            for c in d.get_classes():
                for m in c.get_methods():
                    if (
                        m.get_descriptor() == "(Ljava/lang/Float; Ljava/lang/String;)Ljava/lang/String;"
                    ):
    ...
    def decrypt_files(self, xor_key:bytes):
        ...
        for filepath in self.apk_object.get_files():
            ...
            if self.check_and_write_file(dec):
                ...
                self.logger.info("Writing decrypted file to: %s",self.decrypted_payload_path)
            else:
                try:
                    ...
                    if decrypted.startswith("<!DOCTYPE html>"):
                        ...
                        self.logger.info("Writing decrypted file to: %s",calculated_name)
